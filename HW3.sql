...
CREATE TABLE `profiles` (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) -- что за зверь в целом?
    	ON UPDATE CASCADE -- как это работает?  акие варианты?
    	ON DELETE restrict -- как это работает?  акие варианты?
...
CREATE TABLE `photos` (
	...
	-- ѕроанализировать структуру Ѕƒ vk, которую мы создали на зан€тии, и внести предложени€ по усовершенствованию (если такие идеи есть). Ќапишите пожалуйста, всЄ-ли пон€тно по структуре.
	-- ƒобавить необходимую таблицу/таблицы дл€ того, чтобы можно было использовать лайки дл€ медиафайлов, постов и пользователей.
...
CREATE TABLE likes_users(
	...
	-- »спользу€ сервис http://filldb.info или другой по вашему желанию, сгенерировать тестовые данные дл€ всех таблиц, учитыва€ логику св€зей. ƒл€ всех таблиц, где это имеет смысл, создать не менее 100 строк. —оздать локально Ѕƒ vk и загрузить в неЄ тестовые данные.